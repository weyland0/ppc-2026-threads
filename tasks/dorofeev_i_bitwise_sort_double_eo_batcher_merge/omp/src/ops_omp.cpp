...
bool DorofeevIBitwiseSortDoubleEOBatcherMergeOMP::RunImpl() {
  ...
  // Выносим количество чанков в int, чтобы угодить линтеру со сравнениями типов
  int num_chunks_int = static_cast<int>(num_chunks);

  // 2. Параллельно сортируем каждый блок (явно указываем shared переменные)
#pragma omp parallel for num_threads(num_threads) default(none) shared(chunk_size, raw_data, num_chunks_int)
  ...
  // 3. Собираем отсортированные блоки вместе
