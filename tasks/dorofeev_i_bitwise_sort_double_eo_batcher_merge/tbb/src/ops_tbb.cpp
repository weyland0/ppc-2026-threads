...
// Вынесли обработку отдельного чанка, чтобы снизить когнитивную сложность
...
// Вынесли всю логику TBB арены в отдельную функцию
void ExecuteTBBSort(double *raw_data, size_t pow2, size_t chunk_size, int num_chunks_int, int num_threads) {
  tbb::task_arena arena(num_threads);
  arena.execute([&]() {
    // 1. Параллельно сортируем каждый блок
...
// Теперь метод RunImpl читается как легкая история без страшной вложенности
bool DorofeevIBitwiseSortDoubleEOBatcherMergeTBB::RunImpl() {
  ...
  // Запускаем инкапсулированную логику TBB
  ExecuteTBBSort(raw_data, pow2, chunk_size, num_chunks_int, num_threads);
