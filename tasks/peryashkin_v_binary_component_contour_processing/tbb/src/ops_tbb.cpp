...
inline OutType SolveTBB(const BinaryImage &img) {
...
PeryashkinVBinaryComponentContourProcessingTBB::PeryashkinVBinaryComponentContourProcessingTBB(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput().clear();
}

bool PeryashkinVBinaryComponentContourProcessingTBB::ValidationImpl() {
...
bool PeryashkinVBinaryComponentContourProcessingTBB::PreProcessingImpl() {
  local_out_.clear();
  return true;
}

bool PeryashkinVBinaryComponentContourProcessingTBB::RunImpl() {
  if (!ValidationImpl()) {
    return false;
  }

  local_out_ = SolveTBB(GetInput());
  return true;
}

bool PeryashkinVBinaryComponentContourProcessingTBB::PostProcessingImpl() {
