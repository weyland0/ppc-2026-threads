...
RemizovKDenseMatrixMultiplicationCannonAlgorithmOmp::RemizovKDenseMatrixMultiplicationCannonAlgorithmOmp(
    const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
}

bool RemizovKDenseMatrixMultiplicationCannonAlgorithmOmp::ValidationImpl() {
...
bool RemizovKDenseMatrixMultiplicationCannonAlgorithmOmp::PreProcessingImpl() {
  GetOutput().clear();
  return true;
}

void RemizovKDenseMatrixMultiplicationCannonAlgorithmOmp::MultiplyBlock(const std::vector<std::vector<double>> &a,
...
void RemizovKDenseMatrixMultiplicationCannonAlgorithmOmp::ShiftBlocksLeft(
...
void RemizovKDenseMatrixMultiplicationCannonAlgorithmOmp::ShiftBlocksUp(
    std::vector<std::vector<std::vector<std::vector<double>>>> &matrix_blocks, int block_count) {
    ...
    for (int i = 0; i < block_count; ++i) {
      for (int j = 0; j < block_count; ++j) {
        RemizovKDenseMatrixMultiplicationCannonAlgorithmOmp::MultiplyBlock(a_blocks[i][j], b_blocks[i][j],
                                                                           c_blocks[i][j], block_size);
      }
    }

    if (step < block_count - 1) {
      RemizovKDenseMatrixMultiplicationCannonAlgorithmOmp::ShiftBlocksLeft(a_blocks, block_count);
      RemizovKDenseMatrixMultiplicationCannonAlgorithmOmp::ShiftBlocksUp(b_blocks, block_count);
    }
  }
}

void RemizovKDenseMatrixMultiplicationCannonAlgorithmOmp::InitializeBlocks(
...
bool RemizovKDenseMatrixMultiplicationCannonAlgorithmOmp::RunImpl() {
...
bool RemizovKDenseMatrixMultiplicationCannonAlgorithmOmp::PostProcessingImpl() {
  return true;
}
