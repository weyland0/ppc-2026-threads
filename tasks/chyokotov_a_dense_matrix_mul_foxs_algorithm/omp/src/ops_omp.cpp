...
ChyokotovADenseMatMulFoxAlgorithmOMP::ChyokotovADenseMatMulFoxAlgorithmOMP(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput().clear();
}

bool ChyokotovADenseMatMulFoxAlgorithmOMP::ValidationImpl() {
  return (GetInput().first.size() == GetInput().second.size());
}

bool ChyokotovADenseMatMulFoxAlgorithmOMP::PreProcessingImpl() {
  GetOutput().clear();
  GetOutput().resize(GetInput().first.size(), 0.0);
  return true;
}

int ChyokotovADenseMatMulFoxAlgorithmOMP::CalculateBlockSize(int n) {
  return static_cast<int>(std::sqrt(static_cast<double>(n)));
}

int ChyokotovADenseMatMulFoxAlgorithmOMP::CountBlock(int n, int size) {
  return (n + size - 1) / size;
}

void ChyokotovADenseMatMulFoxAlgorithmOMP::Matmul(std::vector<double> &a, std::vector<double> &b, int n, int istart,
                                                  int iend, int jstart, int jend, int kstart, int kend) {
#pragma omp parallel for collapse(2) default(none) shared(a, b, n, istart, iend, jstart, jend, kstart, kend)
...
bool ChyokotovADenseMatMulFoxAlgorithmOMP::RunImpl() {
...
bool ChyokotovADenseMatMulFoxAlgorithmOMP::PostProcessingImpl() {
  return true;
}
