...
ZorinDStrassenAlgMatrixTBB::ZorinDStrassenAlgMatrixTBB(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput().clear();
}

bool ZorinDStrassenAlgMatrixTBB::ValidationImpl() {
...
bool ZorinDStrassenAlgMatrixTBB::PreProcessingImpl() {
  const auto n = GetInput().n;
  GetOutput().assign(n * n, 0.0);
  return true;
}

bool ZorinDStrassenAlgMatrixTBB::RunImpl() {
  const auto &in = GetInput();
  ...
  (void)control;
  StrassenTopTbb(a_pad.data(), padded, b_pad.data(), padded, c_pad.data(), padded, padded);
...
bool ZorinDStrassenAlgMatrixTBB::PostProcessingImpl() {
  return true;
}
